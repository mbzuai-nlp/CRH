...
- Compute PCA direction (PC1) on training set and test on the test set
...
RESULTS_BASE_PATH = "./baseline_results/exp3_PCA/gemma"
...
def compute_pca_vector_from_training(train_qids, layer, cid, n_components=1):
    """Train PCA vector from the training set"""
    diff_vecs = []
    ...
    flat_vecs = [dv.flatten().cpu().numpy().astype(np.float32) for dv in diff_vecs]
    data = np.stack(flat_vecs, axis=0)
    _, _, vt = np.linalg.svd(data, full_matrices=False)
    pc1 = vt[0]
    pca_vec = torch.from_numpy(pc1).reshape(diff_vecs[0].shape)
    return pca_vec
...
def run_experiment1_pca(model, tokenizer, layer, cid, train_qids, test_qids, df, verbose=True, device=None):
    """PCA vector steering with steer_length batch processing for all questions"""
    if device is None:
        device = next(model.parameters()).device
    diff_root = os.path.dirname(DIFF_VEC_BASE_PATH)
    model_name = os.path.basename(DIFF_VEC_BASE_PATH)
    pca_save_root = os.path.join(diff_root, "pca_vectors", model_name, str(layer))
    os.makedirs(pca_save_root, exist_ok=True)
    results = {
        'cid': cid,
        'layer': layer,
        'experiment_type': 'pca',
    ...
    # Train PCA vector
    if verbose:
        print(f"  Training PCA vector: layer {layer}, cid={cid}, train set size={len(train_qids)}...")
    pca_vec = compute_pca_vector_from_training(train_qids, layer, cid)
    if pca_vec is None:
        if verbose:
            print(f"  Warning: PCA vector not found, skip cid={cid}")
        return None
    
    pca_vec_norm = torch.norm(pca_vec).item()
    if verbose:
        print(f"  PCA vector norm={pca_vec_norm:.4f}")
    results['pca_vec_norm'] = float(pca_vec_norm)
    
    pca_normalized = pca_vec / torch.norm(pca_vec)
    ...
    pca_save_path = os.path.join(pca_save_root, f"{cid}.pt")
    torch.save(
        {
            "pca_vector": pca_vec.cpu(),
            "pca_vec_norm": pca_vec_norm,
    ...
    for qid in test_qids:
        ...
        try:
            diff_vec_norm, sin_theta, cos_theta = compute_angle_components(diff_vec, pca_vec)
    ...
    # Precompute PCA steering vectors for each steer_len_idx (batch generation)
    ...
    for steer_len_idx in range(N_STEPS):
        ...
        steering_vector = pca_normalized * steer_len
    ...
    for config_idx, (steer_len_idx, config_info) in enumerate(batch_configs.items()):
        ...
        try:
            ...
            for q_idx, output in zip(question_indices, outputs):
                ...
                question_results[q_idx]['pca_results'].append({
    ...
    for test_idx, result in question_results.items():
        result['pca_results'].sort(key=lambda x: x['step'])
...
def main():
    ...
    for layer in LAYERS:
        exp1_output_file = f"{RESULTS_BASE_PATH}/exp3_pca_layer{layer}.json"
    ...
    if USE_MULTI_GPU:
        ...
        for layer in LAYERS:
            ...
            # Sub-experiment: PCA
            print(f"\nSub-experiment: PCA (layer {layer})")
            exp1_output_file = f"{RESULTS_BASE_PATH}/exp3_pca_layer{layer}.json"
