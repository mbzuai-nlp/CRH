...
# -*- coding: utf-8 -*-
"""
Experiment 3: Steering with probe vectors
- Run experiments on layers 9 and 13
- Train probe vector on act_with/act_no and test on the test set
...
RESULTS_BASE_PATH = "./baseline_results/exp3_probe/gemma"
...
def compute_probe_vector_from_training(train_qids, layer, cid, max_steps=200, lr=0.1):
    """Train probe vector from act_with/act_no tokens."""
    pos_vecs = []
    neg_vecs = []
    original_shape = None
...
def run_experiment1_probe(model, tokenizer, layer, cid, train_qids, test_qids, df, verbose=True, device=None):
    """Probe vector steering with steer_length batch processing for all questions"""
    if device is None:
        device = next(model.parameters()).device
    # Save{diff_vecs_root}/probe_vectors/{model}/{layer}/{cid}.pt
    diff_root = os.path.dirname(DIFF_VEC_BASE_PATH)  # e.g., CRH_Data/diff_vecs_with_actis
    model_name = os.path.basename(DIFF_VEC_BASE_PATH)  # e.g., gemma2b
    ...
    # Train probe vector
    if verbose:
        print(f"  Training probe vector: layer {layer}, cid={cid}, train set size={len(train_qids)}...")
    probe_vec = compute_probe_vector_from_training(train_qids, layer, cid)
    if probe_vec is None:
        if verbose:
            print(f"  Warning: probe vector not found, skip cid={cid}")
        return None
    ...
    # Save concept probe and diffmean vectors
    ...
    # Precompute probe steering vectors for each steer_len_idx (batch generation)
