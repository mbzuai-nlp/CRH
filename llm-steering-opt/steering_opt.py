...
def make_abl_mat(v):
	...
	This is useful for ablation steering and clamp steering (see Sec. 2 of XXXX), where we want to remove all information in the direction of v from model activations x (ablation steering), or where we want to set the component of x in the direction of v to have a certain value (clamp steering).
...
def optimize_vector(model, datapoints, layer,
	...
	Args:
		...
		Noisy steering args:
			...
			do_antipgd (False by default): if True, then uses anti-correlated noise (see XXXX)
